...
function browserSync(done) {
    browsersync.init({
        files: ['./**.php', './**/**.php', './**/**.twig', './dist/js/**.js', './views/blocks/**/dist/js/**.js'],
...
function sassLint() {
    return gulp
        .src([directories.src + '/**/*.scss', directories.blocks + '/**/dev/sass/*.scss'])
...
function compileThemeSass(done) {
    ...
    compileSass(directories.src + '/sass/styles.scss', directories.dest + '/css', [directories.src + '/base', directories.src + '/components', directories.src + '/layout', directories.src + '/templates']);

    // Compile the editor sass
    compileSass(directories.src + '/sass/editor-styles.scss', directories.dest + '/css', []);
    ...
    var blockDirs = getDirectories(directories.blocks).map(function (blocktype) {
        if (blocktype.length) {
            ...
            if (fs.existsSync(blockpath + '/dev/sass')) {
                compileSass(blockpath + '/dev/sass/**.scss', blockpath + '/dist/css', []);
            }
        }
    });
...
function jsLint() {
    return gulp.src(['**/*.js', '!node_modules/**', '!dist/**', '!**/plugins/**', '!vendor/**']).pipe(eslint()).pipe(eslint.format()).pipe(eslint.failAfterError());
}
...
function compileThemeJs(done) {
    // Compile theme JS
    compileJs([directories.src + '/js/**/*.js', '!' + directories.src + '/js/plugins/*.js'], directories.dest + '/js', 'scripts');

    // Compile the plugins
    compileJs(directories.src + '/js/plugins/*.js', directories.dest + '/js', 'plugins');

    // Compile the vendors
    if (vendors.length > 0) {
        compileJs(vendors, directories.dest + '/js', 'vendor');
    }
    done();
}
    ...
    var blockDirs = getDirectories(directories.blocks).map(function (blocktype) {
        if (blocktype.length) {
            ...
            if (fs.existsSync(blockpath + '/dev/js')) {
                compileJs(blockpath + '/dev/js/**.js', blockpath + '/dist/js', 'scripts');
            }
        }
    });
...
function moveFonts(done) {
    gulp.src(directories.src + '/font/**').pipe(gulp.dest(directories.dest + '/font'));
    done();
}
...
exports.default = gulp.series([setSourcemapStateToDevelopment, cleanupDist, clearCache, sassLint, compileThemeSass, compileBlockSass, jsLint, compileThemeJs, compileBlockJs, moveFonts, minifyImages, minifyBlockImages, browserSync, watchFiles]);

// Use the buildTheme option for version setting in the style.css (first run npm version command)
exports.build = gulp.series([setSourcemapStateToDevelopment, cleanupDist, clearCache, compileThemeSass, compileBlockSass, compileThemeJs, compileBlockJs, moveFonts, minifyImages, minifyBlockImages, buildTheme]);

// Use the buildTheme option for version setting in the style.css (first run npm version command)
exports.buildProduction = gulp.series([setSourcemapStateToProduction, cleanupDist, clearCache, compileThemeSass, compileBlockSass, compileThemeJs, compileBlockJs, moveFonts, minifyImages, minifyBlockImages, buildTheme]);
